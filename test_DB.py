from configuration_DB import conn
...
def test_check_max_salary_value():
    ...
    """
    cursor.execute(f""" SELECT max([salary]) as max_sal 
...
def test_check_country_values():
    ...
    """
    cursor.execute(f""" SELECT DISTINCT [country_id] as id
...
def test_check_duplicates_departments_tbl():
    ...
    row = cursor.fetchone()
    assert row['cnt_all'] == row['cnt_unq']
